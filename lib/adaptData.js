function normalizeNext(nextActions) {
  ...
  // Keys may be BF/BC/BR (vs bet) or RF/RC/RR (vs raise) — match by suffix
  const keys = Object.keys(nextActions);
  const foldKey = keys.find(k => k.endsWith('F'));
  const callKey = keys.find(k => k.endsWith('C'));
  const raiseKey = keys.find(k => k !== foldKey && k !== callKey);
